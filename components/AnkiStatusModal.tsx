      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-md w-full overflow-hidden animate-fade-in border border-slate-200 dark:border-slate-700 transition-colors duration-300">
        <div className="p-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-xl font-semibold text-slate-900 dark:text-white">{t.connectionSetup}</h3>
          ...
          {isConnected ? (
            <div className="flex flex-col items-center py-6 text-center">
              ...
              <h4 className="text-lg font-medium text-green-700 dark:text-green-400 mb-2">{t.successConnected}</h4>
              <p className="text-slate-600 dark:text-slate-300">{t.successDesc}</p>
            </div>
          ) : (
            <div className="space-y-4">
                ...
                <div className="flex">
                  ...
                  <div className="ml-3">
                    <p className="text-sm text-amber-700 dark:text-amber-400">
                      {t.connectionFailed} <code className="bg-amber-100 dark:bg-amber-900/40 px-1 rounded">http://127.0.0.1:8765</code>
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="text-sm text-slate-600 dark:text-slate-300 space-y-3">
                <p className="font-medium text-slate-800 dark:text-slate-200">{t.enableConnection}</p>
...
{`{
    "webCorsOriginList": [
        ...
        "*" 
                    ...
                    <span className="text-xs text-slate-400 block mt-1">{t.tip}</span>
                  </li>
                </ol>
              </div>
            </div>
